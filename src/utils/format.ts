...
export const foreachField = (obj:Object, predicate = (key:string, val:any, i:number) => {}) => {
...
export const mapFields = (obj:Object, predicate:(key:string, val:any, i:number)=>Object = (key, val, i) => val) => {
    const keys = Object.keys(obj);
    const values = Object.values(obj);
    const res:any = {};
    for(let i = 0; i < keys.length; i++){
...
export const functionConstructor = <T,>(saveObject:T|undefined, callback:(...callbackArgs:any[])=>T|undefined, ...callbackArgs: any[]) => {
    (() => {
        if(saveObject)
        saveObject = callback(...callbackArgs);
    })();
    return saveObject;
}
